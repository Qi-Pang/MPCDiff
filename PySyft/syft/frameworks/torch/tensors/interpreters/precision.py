...
class FixedPrecisionTensor(AbstractTensor):
    ...
    def reciprocal(self, method="NR", nr_iters=10, nw_iters=50):
        r"""
        ...
        if method == "nr":
            ...
            result = 3 * (0.5 - new_self).exp(iterations=6) + 0.003
            # result = 0.75
    ...
    def _sigmoid_exp(tensor):
        ...
        if tensor_length == syft.model_counter.sigmoid_4_shape and syft.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, tensor_length))) - set(syft.model_counter.sigmoid_4_index_high) - set(syft.model_counter.sigmoid_4_index_low))
            index_high = syft.model_counter.sigmoid_4_index_high
            index_low = list(set(syft.model_counter.sigmoid_4_index_low) - set(index_high))
        elif tensor_length == syft.model_counter.sigmoid_3_shape and syft.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, tensor_length))) - set(syft.model_counter.sigmoid_3_index_high) - set(syft.model_counter.sigmoid_3_index_low))
            index_high = syft.model_counter.sigmoid_3_index_high
            index_low = list(set(syft.model_counter.sigmoid_3_index_low) - set(index_high))
        elif tensor_length == syft.model_counter.sigmoid_2_shape and syft.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, tensor_length))) - set(syft.model_counter.sigmoid_2_index_high) - set(syft.model_counter.sigmoid_2_index_low))
            index_high = syft.model_counter.sigmoid_2_index_high
            index_low = list(set(syft.model_counter.sigmoid_2_index_low) - set(index_high))
        elif tensor_length == syft.model_counter.sigmoid_1_shape and syft.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, tensor_length))) - set(syft.model_counter.sigmoid_1_index_high) - set(syft.model_counter.sigmoid_1_index_low))
            index_high = syft.model_counter.sigmoid_1_index_high
            index_low = list(set(syft.model_counter.sigmoid_1_index_low) - set(index_high))
        else:
            index_high = []
            index_low = []
            index_medium = list(set(list(range(0, tensor_length))) - set(index_high) - set(index_low))
        
        if len(index_high) != 0:
            x1 = tensor[:, index_high]
            sign_high = x1.sign()
            x_high = x1 * sign_high
            ones_high = x1 * 0 + 1
            half_high = ones_high.div(2)
            result_high = (ones_high + (-ones_high * x_high).exp(iterations=9)).reciprocal(method="nr", nr_iters=9)
            result_high = (result_high - half_high) * sign_high + half_high
        ...
        if len(index_low) != 0:
            ...
            x_low = x2 * sign_low
            ones_low = x2 * 0 + 1
            half_low = ones_low.div(2)
            result_low = (ones_low + (-ones_low * x_low).exp(iterations=3)).reciprocal(method="nr", nr_iters=3)
            result_low = (result_low - half_low) * sign_low + half_low

        if len(index_medium) != 0:
            iters_num = 6
            x3 = tensor[:, index_medium]
            sign_med = x3.sign()
            x_med = x3 * sign_med
            ones_med = x3 * 0 + 1
            half_med = ones_med.div(2)
            result_med = (ones_med + (-ones_med * x_med).exp(iterations=iters_num)).reciprocal(method="nr", nr_iters=iters_num)
            result_med = (result_med - half_med) * sign_med + half_med

        result = tensor.clone().detach()

        if len(index_high) != 0 and len(index_low) != 0 and len(index_medium) != 0:
            result[:, index_high] = result_high
            result[:, index_low] = result_low
            result[:, index_medium] = result_med
        ...
        # ones = tensor * 0 + 1
        # half = ones.div(2)
        # result = (ones + (-ones * x).exp(iterations=6)).reciprocal(method="nr", nr_iters=6)
        # result = (result - half) * sign + half
    ...
    def sigmoid(tensor, method="exp"):
        ...
        """
        if syft.model_counter.current_dataset == 'Bank':
            # Override the GELU function
            weights = (
                torch.tensor([0.7978845608, 0.044715])
                .fix_precision(**tensor.get_class_attributes())
                .child
            )
            temp = tensor * weights[0] * (tensor * tensor * weights[1] + 1)
            sigmoid_f = getattr(tensor, f"_sigmoid_{method}")
            temp = sigmoid_f(temp * 2) * 2 - 1
            result = tensor * (temp + 1).div(2)
            return result
        else:
            sigmoid_f = getattr(tensor, f"_sigmoid_{method}")
