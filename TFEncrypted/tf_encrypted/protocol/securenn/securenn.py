...
class SecureNN(Pond):
  ...
  """
    ...
    @memoize
    def reciprocal_nn(self, x, nr_iters=1):
        initial = 0.75
        if initial is None:
    ...
    def sigmoid(self, x, **kwargs):
        original_shape = x.shape.as_list()
        x = x.reshape((-1, 1))
        print(x.shape, type(original_shape))
        temp_x = tfe.identity(x)
        if x.shape[0] == tfe.model_counter.sigmoid_4_shape and tfe.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, x.shape[0]))) - set(tfe.model_counter.sigmoid_4_index_high) - set(tfe.model_counter.sigmoid_4_index_low))
            index_high = tfe.model_counter.sigmoid_4_index_high
            index_low = list(set(tfe.model_counter.sigmoid_4_index_low) - set(index_high))
        elif x.shape[0] == tfe.model_counter.sigmoid_3_shape and tfe.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, x.shape[0]))) - set(tfe.model_counter.sigmoid_3_index_high) - set(tfe.model_counter.sigmoid_3_index_low))
            index_high = tfe.model_counter.sigmoid_3_index_high
            index_low = list(set(tfe.model_counter.sigmoid_3_index_low) - set(index_high))
        elif x.shape[0] == tfe.model_counter.sigmoid_2_shape and tfe.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, x.shape[0]))) - set(tfe.model_counter.sigmoid_2_index_high) - set(tfe.model_counter.sigmoid_2_index_low))
            index_high = tfe.model_counter.sigmoid_2_index_high
            index_low = list(set(tfe.model_counter.sigmoid_2_index_low) - set(index_high))
        elif x.shape[0] == tfe.model_counter.sigmoid_1_shape and tfe.model_counter.global_repair_flag:
            index_medium = list(set(list(range(0, x.shape[0]))) - set(tfe.model_counter.sigmoid_1_index_high) - set(tfe.model_counter.sigmoid_1_index_low))
            index_high = tfe.model_counter.sigmoid_1_index_high
            index_low = list(set(tfe.model_counter.sigmoid_1_index_low) - set(index_high))
        else:
            index_high = []
            index_low = []
            index_medium = list(set(list(range(0, x.shape[0]))) - set(index_high) - set(index_low))

        if len(index_high) != 0:
            x1 = tfe.gather(temp_x, index_high)
            ltz_high = self.is_negative(x1)
            sign_high = 1 - 2 * ltz_high
            pos_input_high = x1 * sign_high
            denominator_high = self.exp(0-pos_input_high, iters=5) + 1
            pos_output_high = self.reciprocal_nn(denominator_high, nr_iters=4)
            result_high = self.select(ltz_high, pos_output_high, 1 - pos_output_high)
        if len(index_low) != 0:
            x2 = tfe.gather(temp_x, index_low)
            ltz_low = self.is_negative(x2)
            sign_low = 1 - 2 * ltz_low
            pos_input_low = x2 * sign_low
            denominator_low = self.exp(0-pos_input_low, iters=3) + 1
            pos_output_low = self.reciprocal_nn(denominator_low, nr_iters=2)
            result_low = self.select(ltz_low, pos_output_low, 1 - pos_output_low)
        if len(index_medium) != 0:
            x3 = tfe.gather(temp_x, index_medium)
            ltz_medium = self.is_negative(x3)
            sign_medium = 1 - 2 * ltz_medium
            pos_input_medium = x3 * sign_medium
            denominator_medium = self.exp(0-pos_input_medium, iters=4) + 1
            pos_output_medium = self.reciprocal_nn(denominator_medium, nr_iters=1)
            result_medium = self.select(ltz_medium, pos_output_medium, 1 - pos_output_medium)

        if len(index_high) != 0 and len(index_low) != 0 and len(index_medium) != 0:
            result = tfe.concat([result_high, result_low, result_medium], axis=0)
        elif len(index_low) == 0 and len(index_medium) != 0 and len(index_high) != 0:
            result = tfe.concat([result_high, result_medium], axis=0)
        elif len(index_low) == 0 and len(index_medium) == 0:
            result = result_high
        elif len(index_high) == 0 and len(index_low) == 0:
            result = result_medium
        elif len(index_high) != 0 and len(index_low) != 0 and len(index_medium) == 0:
            result = tfe.concat([result_high, result_low], axis=0)

        concat_index = index_high + index_low + index_medium
        reshape_list = [None] * len(concat_index)
        for r_l in range(len(concat_index)):
            reshape_list[concat_index[r_l]] = r_l
        result = tfe.gather(result, reshape_list)
        # ltz = self.is_negative(x)
        # sign = 1 - 2 * ltz

        # pos_input = x * sign
        # denominator = self.exp(0-pos_input) + 1

        # # TODO: Set these with configurable parameters
        # pos_output = self.reciprocal(denominator)
        # result = self.select(ltz, pos_output, 1 - pos_output)
        result = result.reshape(original_shape)
        return result

        # tanh_approx = self.tanh(x / 2)
        # tanh_approx = self.relu((tanh_approx + 1)) - self.relu((tanh_approx - 1)) - 1
        # with tf.name_scope("relu"):
        #     z = tanh_approx / 2 + 0.5
        #     return z

    @memoize
    def tanh_nn(self, x, **kwargs):
        result = self.sigmoid(x * 2.0) * 2.0 - 1.0
        return result

    @memoize
    def gelu(self, x, **kwargs):
        # gelu function
        result = x * (self.tanh_nn(x * 0.7978845608 * (x * x * 0.044715 + 1.0)) + 1.0) * 0.5
        return result
