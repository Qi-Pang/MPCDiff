...
class Pond(Protocol):
  ...
  """  # noqa:E501
    ...
    def update_with_index(self, x: "PondTensor", indices: list, y:"PondTensor") -> "PondTensor":
        x0, x1 = x.unwrapped
        y0, y1 = y.unwrapped

        with tf.name_scope("update_with_index"):
            with tf.device(self.server_0.device_name):
                # x0.value[indices] = y0.value
                for i in range(len(indices)):
                    print(indices[i], x0.value.shape, y0.value.shape)
                    x0.value = x1.value
                    # x0.value[indices[i], 0].assign(y0.value[i, 0])
                # x0.value = tf.tensor_scatter_nd_update(x0.value, indices, y0.value)

            with tf.device(self.server_1.device_name):
                x1 = tf.tensor_scatter_nd_update(x1.value, indices, y1.value)

        return PondPrivateTensor(self, x0, x1, x.is_scaled)
    ...
    def sigmoid(self, x: "PondTensor"):
        ...
        with tf.name_scope("sigmoid"):
            ...
            # z = y9 + y7 + y5 + y3 + y1 + w0
            # z = y7 + y5 + y3 + y1 + w0
        # tf.Print(z.reveal().to_native(), [z.reveal().to_native()], summarize=-1)
        return z
    ...
    """
        ...
        terms = 10
        coeffs = self.chebyshev_series(np.tanh, 1, terms)[1::2]
        ...
        with tf.name_scope("tanh"):
            tanh_polys = self._chebyshev_polynomials(x, terms)
            z = tanh_polys[0] * coeffs[0]
            for index_coeff in range(1, len(coeffs)):
                z = z + tanh_polys[index_coeff] * coeffs[index_coeff]
            # x1 = x
            # x2 = x.square()
            # x3 = x2 * x1
            # x5 = x2 * x3
    ...
    def chebyshev_series(self, func, width, terms):
        ...
        """
        n_range = np.arange(start=0, stop=terms)
    ...
    def _chebyshev_polynomials(self, x, terms):
        # x = x.flatten()
        polynomials = [x]
        y = 4 * x * x - 2
        z = y - 1
        ...
        polynomials = self.stack(polynomials)
        return polynomials
...
# def _reciprocal_private(prot, x):
#     initial = 0.75
#     nr_iters = 1
#     if initial is None:
