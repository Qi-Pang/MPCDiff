...
def urandom_from_random(rng, length):
    return rng.getrandbits(8 * length).to_bytes(length, 'big')
...
def _setup_prng():
    ...
    seed = rng.randint(-2**63, 2**63)
    # seed = int.from_bytes(urandom_from_random(rng, 8), "big") - 2 ** 63
    ...
    # local_seed = int.from_bytes(urandom_from_random(rng, 8), "big") - 2 ** 63
    local_seed = rng.randint(-2**63, 2**63)
    ...
    # global_seed = int.from_bytes(urandom_from_random(rng, 8), "big") - 2 ** 63
    global_seed = rng.randint(-2**63, 2**63)
