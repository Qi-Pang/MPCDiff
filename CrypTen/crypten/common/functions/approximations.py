...
def inv_sqrt(self):
    ...
    self = self.flatten()
    total_num = self.shape[0]
    high_flag = False
    low_flag = False
    med_flag = False
    if model_counter.global_repair_flag:
        if total_num == model_counter.batchnorm_3_shape:
            index_high = model_counter.batchnorm_3_index_high
            index_low = model_counter.batchnorm_3_index_low
            index_med = list(set(list(range(0, total_num))) - set(index_high) - set(index_low))
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
            if len(index_low) > 0:
        ...
        elif total_num == model_counter.batchnorm_4_shape:
            index_high = model_counter.batchnorm_4_index_high
            index_low = model_counter.batchnorm_4_index_low
            index_med = list(set(list(range(0, total_num))) - set(index_high) - set(index_low))
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
    ...
    if repair_flag and model_counter.global_repair_flag:
        temp_output = self.clone()
        if high_flag:
            data_high = self[index_high]
        if med_flag:
            data_med = self[index_med]
        if low_flag:
            data_low = self[index_low]
        if initial is None:
        ...
        if low_flag:
            for _ in range(iters):
                y_low = y_low.mul_(3 - data_low * y_low.square()).div_(2)
            temp_output[index_low] = y_low
        if med_flag:
            for _ in range(iters):
                y_med = y_med.mul_(3 - data_med * y_med.square()).div_(2)
            temp_output[index_med] = y_med
        if high_flag:
            for _ in range(iters*2):
                y_high = y_high.mul_(3 - data_high * y_high.square()).div_(2)
            temp_output[index_high] = y_high
        # if total_num == model_counter.batchnorm_4_shape:
        #     new_encoder = crypten.encoder.FixedPointEncoder(precision_bits=16)
        #     temp_output = temp_output * new_encoder._scale / temp_output.encoder._scale
        #     temp_output.encoder = new_encoder
        return temp_output.reshape(orig_shape)

    # Initialize using decent approximation
    else:
        temp_output = self.clone()
        index_high = list(range(0, total_num//3))
        index_low = list(range(2*total_num//3, total_num))
        index_med = list(set(list(range(0, total_num))) - set(index_high) - set(index_low))
...
def sigmoid(self):
    ...
    if model_counter.global_repair_flag:
        if total_num == model_counter.sigmoid_1_shape:
            index_high = model_counter.sigmoid_1_index_high
            index_low = model_counter.sigmoid_1_index_low
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
            if len(index_low) > 0:
        ...
        elif total_num == model_counter.sigmoid_2_shape:
            index_high = model_counter.sigmoid_2_index_high
            index_low = model_counter.sigmoid_2_index_low
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
            if len(index_low) > 0:
        ...
        elif total_num == model_counter.sigmoid_3_shape:
            index_high = model_counter.sigmoid_3_index_high
            index_low = model_counter.sigmoid_3_index_low
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
            if len(index_low) > 0:
        ...
        elif total_num == model_counter.sigmoid_4_shape:
            index_high = model_counter.sigmoid_4_index_high
            index_low = model_counter.sigmoid_4_index_low
            # new_encoder = FixedPointEncoder(precision_bits=16)
            # self = self * new_encoder._scale / self.encoder._scale
            # self.encoder = FixedPointEncoder(precision_bits=16)
            if len(index_high) > 0:
                repair_flag = True
                high_flag = True
            if len(index_low) > 0:
    ...
    if repair_flag and model_counter.global_repair_flag:
        ...
        if high_flag:
            data_high = self[index_high]
            denominator_high = denominator[index_high]
        if med_flag:
            data_med = self[index_med]
            denominator_med = denominator[index_med]
        if low_flag:
            data_low = self[index_low]
            denominator_low = denominator[index_low]
        pos_output = self.clone()
            ...
            with cfg.temp_override(
                {
                    ...
                    "functions.reciprocal_nr_iters": 9,
                    "functions.reciprocal_all_pos": True,
                    "functions.reciprocal_initial": 0.75,
                }):
                # ltz_high = data_high._ltz()
                # sign_high = 1 - 2 * ltz_high
                # pos_input_high = data_high.mul(sign_high)
                denominator_high = denominator_high.exp().add(1)
                pos_output_high = denominator_high.reciprocal()
                pos_output[index_high] = pos_output_high
                # result_high = pos_output_high.where(1 - ltz_high, 1 - pos_output_high)
        # HACK: med
        if med_flag:
            with cfg.temp_override(
                {
                    "functions.exp_iterations": 6,
                    "functions.reciprocal_nr_iters": 6,
                    "functions.reciprocal_all_pos": True,
                    "functions.reciprocal_initial": 0.75,
                }):
                # ltz_med = data_med._ltz()
                # sign_med = 1 - 2 * ltz_med
                # pos_input_med = data_med.mul(sign_med)
                denominator_med = denominator_med.exp().add(1)
                pos_output_med = denominator_med.reciprocal()
                pos_output[index_med] = pos_output_med
                # result_med = pos_output_med.where(1 - ltz_med, 1 - pos_output_med)
        if low_flag:
            # HACK: low
            with cfg.temp_override(
                {
                    "functions.exp_iterations": 3,
                    "functions.reciprocal_nr_iters": 3,
                    "functions.reciprocal_all_pos": True,
                    "functions.reciprocal_initial": 0.75,
                }):
                # ltz_low = data_low._ltz()
                # sign_low = 1 - 2 * ltz_low
                # pos_input_low = data_low.mul(sign_low)
                denominator_low = denominator_low.exp().add(1)
                pos_output_low = denominator_low.reciprocal()
                pos_output[index_low] = pos_output_low
                # result_low = pos_output_low.where(1 - ltz_low, 1 - pos_output_low)

        result = pos_output.where(1 - ltz, 1 - pos_output)
        end_time = time.time()
        temprank = comm.get().get_rank()
        # if temprank == 1:
        #     model_counter.computation_time += (end_time - start_time)
            # HACK: time here
            # print(model_counter.computation_time)

        return result.reshape(orig_shape)
        '''
        # HACK: compare with highest
        start_time = time.time()
        with cfg.temp_override(
            {
                "functions.exp_iterations": 12,
                "functions.reciprocal_nr_iters": 12,
                "functions.reciprocal_all_pos": True,
                "functions.reciprocal_initial": 0.75,
            }):
            ltz_high = self._ltz()
            sign_high = 1 - 2 * ltz_high
            pos_input_high = self.mul(sign_high)
            denominator_high = pos_input_high.neg().exp().add(1)
            pos_output_high = denominator_high.reciprocal()
            result_high = pos_output_high.where(1 - ltz_high, 1 - pos_output_high)
        end_time = time.time()
        temprank = comm.get().get_rank()
        if temprank == 1:
            model_counter.computation_time += (end_time - start_time)
            print(model_counter.computation_time)
        return result_high.reshape(orig_shape)
        '''
...
def sigmoid(self):
    ...
    orig_shape = self.shape
    self = self.flatten()
    if temprank == 1 and model_counter.count_sigmoid == 1:
        # A = self.clone().flatten().reshape(orig_shape)
        index1 = [0, 299, 700]
        index2 = [1, 300, 701]
        self[index1] = self[index2]
        print("called sigmoid", self[index1], self[index2], model_counter.count_sigmoid)
    if method == "chebyshev":
        tanh_approx = tanh(self.div(2))
        return tanh_approx.div(2) + 0.5
    elif method == "reciprocal":
...
def softplus(self):
    result = (self.exp() + 1).log()
    return result

def elu(self):
    # Override the GELU function
    result = 0.5 * self * (1 + tanh(self * 0.7978845608 * (1 + 0.044715 * self * self)))
    return result
