        ...
        if mode != self.encrypted:
            ...
            for name, param in self.named_parameters(recurse=False):
                ...
                if mode:  # encrypt parameter
                    # if param.shape[0] != 10:
                    #     cfg.encoder.precision_bits = 12
                    #     temp_cryptensor = crypten.cryptensor(
                    #         param, **{"src": src}, requires_grad=requires_grad
                    #     )
                    #     cfg.encoder.precision_bits = 16
                    #     new_encoder = crypten.encoder.FixedPointEncoder(precision_bits=16)
                    #     temp_cryptensor = temp_cryptensor * new_encoder._scale / temp_cryptensor.encoder._scale
                    #     temp_cryptensor.encoder = new_encoder
                    # else:
                    temp_cryptensor = crypten.cryptensor(
                        param, **{"src": src}, requires_grad=requires_grad
                    )
...
class Sigmoid(Module):
    ...
    def forward(self, x):
        # temprank = comm.get().get_rank()
        # model_counter.count_sigmoid += 0.5
            # print("x.sigmoid here")
        return x.sigmoid()
